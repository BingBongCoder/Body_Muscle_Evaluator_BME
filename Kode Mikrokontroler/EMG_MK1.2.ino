...
void setup() {
    ...
    for(int i = 0; i < initNumSamples; i++) {
        initVsum += analogReadMilliVolts(batteryPin);
        delay(1); 
    }
    
    smoothedVbatt = (2.0 * initVsum / (float)initNumSamples) / 1000.0;
...
void loop() {
    ...
    if (currentMillis - lastBatteryMillis >= 1000) {
        ...
        float currentVbatt = (2.0 * analogReadMilliVolts(batteryPin)) / 1000.0;
    ...
    if (systemActive && WiFi.status() == WL_CONNECTED) {
        if (!client.connected()) {
            ...
            if (client.available()) {
                ...
                if (request.startsWith("MOS:1")) {
                    updateMosfet(true);
                } else if (request.startsWith("MOS:0")) {
                    updateMosfet(false);
                } else if (request.startsWith("PWM1:")) {
                    analogWrite(ledPin1, request.substring(5).toInt());
                } else if (request.startsWith("PWM2:")) {
                    analogWrite(ledPin2, request.substring(5).toInt());
                } else if (request.startsWith("PWM3:")) {
                    analogWrite(ledPin3, request.substring(5).toInt());
                }
                ...
                if (request.startsWith(cmd_srR)) { 
                    srR = request.substring(cmd_srR.length()).toInt(); 
                    pref.putInt("srR", srR); 
                } else if (request.startsWith(cmd_srG)) { 
                    srG = request.substring(cmd_srG.length()).toInt(); 
                    pref.putInt("srG", srG); 
                } else if (request.startsWith(cmd_srB)) { 
                    srB = request.substring(cmd_srB.length()).toInt(); 
                    pref.putInt("srB", srB); 
                } else if (request.startsWith(cmd_cnR)) { 
                    cnR = request.substring(cmd_cnR.length()).toInt(); 
                    pref.putInt("cnR", cnR); 
                    setLED(cnR, cnG, cnB); 
                } else if (request.startsWith(cmd_cnG)) { 
                    cnG = request.substring(cmd_cnG.length()).toInt(); 
                    pref.putInt("cnG", cnG); 
                    setLED(cnR, cnG, cnB); 
                } else if (request.startsWith(cmd_cnB)) { 
                    cnB = request.substring(cmd_cnB.length()).toInt(); 
                    pref.putInt("cnB", cnB); 
                    setLED(cnR, cnG, cnB); 
                }
            }
            ...
            if (currentMicros - previousMicros >= intervalMicros) {
                ...
                float emgMV = (notchedValue / 4095.0) * 3300.0;
